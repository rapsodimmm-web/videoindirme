...
import com.swenauk.mainmenu.GetsPack.GetIzle720p;
import com.swenauk.seyirturk.R;
import java.util.Map;

/* loaded from: classes3.dex */
public class Izle720p extends Parsers {
    public boolean isAlt;
    public String mainUrl;
    public String oneLinkWonder;
    public String parsed;

    public Izle720p(String str, Context context, ExoPlayer exoPlayer) {
    ...
    protected void parse(String str) {
        ...
        new GetIzle720p(this).execute(str);
    }
    ...
    public void showAlternates(final Map<String, String> map) {
        if (!((Activity) this.calledContext).isDestroyed()) {
            if (map.size() > 1) {
                ...
                builder.setItems(charSequenceArr, new DialogInterface.OnClickListener() { // from class: com.swenauk.mainmenu.Parsers.Izle720p.1
                    @Override // android.content.DialogInterface.OnClickListener
                    public void onClick(DialogInterface dialogInterface, int r4) {
                        Izle720p.this.mainUrl = (String) map.get(charSequenceArr[r4]);
                        Izle720p.this.isAlt = false;
                        new GetIzle720p(Izle720p.this).execute(Izle720p.this.mainUrl);
                    }
                });
                ...
                this.isAlt = false;
                new GetIzle720p(this).execute(this.oneLinkWonder);
            }
        }
        this.isAlt = false;
    }
    ...
    protected void showVideo() {
        if (this.streamUrl.contains(".mp4") && !this.streamUrl.contains(".m3u8")) {
            if (this.streamUrl.contains("yandex.ru")) {
                ...
                this.mediaSource = new ProgressiveMediaSource.Factory(new DataSource.Factory() { // from class: com.swenauk.mainmenu.Parsers.Izle720p.2
                    ...
                    public DataSource createDataSource() {
                        ...
                        if (Izle720p.this.streamUrl.contains("molyusercontent")) {
                            defaultHttpDataSource.setRequestProperty("Referer", Izle720p.this.videoUri.getScheme() + "://" + Izle720p.this.videoUri.getHost());
                        } else {
                            defaultHttpDataSource.setRequestProperty("Referer", Izle720p.this.videoUri.getScheme() + "://" + Izle720p.this.videoUri.getHost());
                        }
            ...
            this.mediaSource = new HlsMediaSource.Factory(new DataSource.Factory() { // from class: com.swenauk.mainmenu.Parsers.Izle720p.3
                @Override // com.google.android.exoplayer2.upstream.DataSource.Factory
                public DataSource createDataSource() {
                    DefaultHttpDataSource defaultHttpDataSource = new DefaultHttpDataSource("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.169 Safari/537.36");
                    String str = Izle720p.this.mainUrl;
                    if (Izle720p.this.streamUrl.contains("molystream")) {
                        str = Izle720p.this.videoUri.getScheme() + "://" + Izle720p.this.videoUri.getHost();
                    }
    ...
    private void createAlertWItems(String str) {
        ...
        builder.setItems(charSequenceArr, new DialogInterface.OnClickListener() { // from class: com.swenauk.mainmenu.Parsers.Izle720p.4
            @Override // android.content.DialogInterface.OnClickListener
            public void onClick(DialogInterface dialogInterface, int r4) {
                Izle720p.this.showBuffer();
                System.out.println(Izle720p.this.streamUrls.get(charSequenceArr[r4]));
                Izle720p izle720p = Izle720p.this;
                izle720p.videoUri = Uri.parse(izle720p.streamUrls.get(charSequenceArr[r4]));
                DefaultDataSourceFactory defaultDataSourceFactory = new DefaultDataSourceFactory(Izle720p.this.calledContext, Util.getUserAgent(Izle720p.this.calledContext, "iFrame"));
                Izle720p.this.mediaSource = new ProgressiveMediaSource.Factory(defaultDataSourceFactory).createMediaSource(MediaItem.fromUri(Izle720p.this.videoUri));
                Izle720p.this.playVideo();
            }
        });
        this.alert = builder.create();
        this.alert.show();
    ...
    public void loadContentx() {
        new Contentx(this.mainUrl, this.calledContext, this.player, this.mainUrlForReferer);
    }

    public void loadFilmakinesi() {
        new Izle720p(this.mainUrl, this.calledContext, this.player);
    }
