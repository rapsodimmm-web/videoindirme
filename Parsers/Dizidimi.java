...
import com.swenauk.mainmenu.GetsPack.GetDizidimi;
import com.swenauk.seyirturk.R;
import java.util.Map;

/* loaded from: classes3.dex */
public class Dizidimi extends Parsers {
    public boolean isAlt;
    public String mainUrl;
    public String oneLinkWonder;
    public String parsed;

    public Dizidimi(String str, Context context, ExoPlayer exoPlayer, DefaultTrackSelector defaultTrackSelector) {
        super(str, context, exoPlayer);
        this.trackSelector = defaultTrackSelector;
    }

    public Dizidimi(String str, Context context, ExoPlayer exoPlayer) {
        super(str, context, exoPlayer);
    }
    ...
    protected void parse(String str) {
        this.isAlt = true;
        new GetDizidimi(this).execute(str);
    }
    ...
    public void showAlternates(final Map<String, String> map) {
        if (!((Activity) this.calledContext).isDestroyed()) {
            if (map.size() > 1) {
                ...
                builder.setItems(charSequenceArr, new DialogInterface.OnClickListener() { // from class: com.swenauk.mainmenu.Parsers.Dizidimi.1
                    @Override // android.content.DialogInterface.OnClickListener
                    public void onClick(DialogInterface dialogInterface, int r4) {
                        Dizidimi.this.mainUrl = (String) map.get(charSequenceArr[r4]);
                        Dizidimi.this.isAlt = false;
                        new GetDizidimi(Dizidimi.this).execute(Dizidimi.this.mainUrl);
                    }
                });
    ...
    protected void showVideo() {
        if (this.streamUrl.contains(".mp4") && !this.streamUrl.contains(".m3u8")) {
            if (this.streamUrl.contains("yandex.ru")) {
                ...
                this.mediaSource = new ProgressiveMediaSource.Factory(new DataSource.Factory() { // from class: com.swenauk.mainmenu.Parsers.Dizidimi.2
                    ...
                    public DataSource createDataSource() {
                        ...
                        if (Dizidimi.this.streamUrl.contains("molyusercontent")) {
                            defaultHttpDataSource.setRequestProperty("Referer", Dizidimi.this.videoUri.getScheme() + "://" + Dizidimi.this.videoUri.getHost());
                        } else {
                            defaultHttpDataSource.setRequestProperty("Referer", Dizidimi.this.videoUri.getScheme() + "://" + Dizidimi.this.videoUri.getHost());
                        }
            ...
            this.mediaSource = new HlsMediaSource.Factory(new DataSource.Factory() { // from class: com.swenauk.mainmenu.Parsers.Dizidimi.3
                @Override // com.google.android.exoplayer2.upstream.DataSource.Factory
                public DataSource createDataSource() {
                    DefaultHttpDataSource defaultHttpDataSource = new DefaultHttpDataSource("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/74.0.3729.169 Safari/537.36");
                    String str = Dizidimi.this.videoUri.getScheme() + "://" + Dizidimi.this.videoUri.getHost();
                    if (Dizidimi.this.streamUrl.contains("molystream")) {
                        str = Dizidimi.this.videoUri.getScheme() + "://" + Dizidimi.this.videoUri.getHost();
                    }
    ...
    private void createAlertWItems(String str) {
        ...
        builder.setItems(charSequenceArr, new DialogInterface.OnClickListener() { // from class: com.swenauk.mainmenu.Parsers.Dizidimi.4
            @Override // android.content.DialogInterface.OnClickListener
            public void onClick(DialogInterface dialogInterface, int r4) {
                Dizidimi.this.showBuffer();
                System.out.println(Dizidimi.this.streamUrls.get(charSequenceArr[r4]));
                Dizidimi dizidimi = Dizidimi.this;
                dizidimi.videoUri = Uri.parse(dizidimi.streamUrls.get(charSequenceArr[r4]));
                DefaultDataSourceFactory defaultDataSourceFactory = new DefaultDataSourceFactory(Dizidimi.this.calledContext, Util.getUserAgent(Dizidimi.this.calledContext, "iFrame"));
                Dizidimi.this.mediaSource = new ProgressiveMediaSource.Factory(defaultDataSourceFactory).createMediaSource(MediaItem.fromUri(Dizidimi.this.videoUri));
                Dizidimi.this.playVideo();
            }
        });
        this.alert = builder.create();
        this.alert.show();
    ...
    public void loadFilmakinesi() {
        new Dizidimi(this.mainUrl, this.calledContext, this.player);
    }
    ...
    public void loadCloseLoad() {
        new CloseLoad(this.mainUrl, this.calledContext, this.player, this.trackSelector);
    }
